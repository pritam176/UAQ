...
public final class ConfUtils {
	...
	private static String[] propertyFileList = { "jdbc.properties", "Application.properties"};
	...
	public static String getValue(String key) {
		...
		try {
			...
			System.out.println("Error while instantiating the class");
		}
		if (value == null) {
			System.out.println("Property value not found for the key " + key);
		}
		return value;
	}
	...
	public Properties loadFromClassPath(String propertiesFileName) {
		...
		try {
			...
			System.out.println("Error while loading the property file :"
					+ propertiesFileName + " : exception = " +  exception);
		}
		return property;
	}
	...
	public Properties loadFromClassPath(String propertiesFileName, String locale) {
		...
		try {
			...
			System.out.println("Error while loading the property file :"
					+ propertiesFileName + "_" + locale + " : exception = " + exception);
		}
		return property;
	}
	...
	public Properties loadFromFile(File file) {
		...
		try {
			...
			System.out.println("Could not load the file :" + file.getName());
		}
		return prop;
	}
	...
	private void loadAllFromClassPath() throws UAQException {
		...
		for (int index = 0; index < propertyFileList.length; index++) {
			System.out.println("Loading ..." + propertyFileList[index]);
