		...
		<div class="col-lg-9 col-sm-9 col-md-9 col-xs-12">
			...
			<div class="page-title-mob">
				...
				<form:form method="POST" id="eventForm" name="eventForm"
					commandName="searchCommand" action="events.html">
					<div class="filter-wrapper hide-on-mobile">
						...
						<div class="filter-date pull-left">
							...
							<form:input path="startDate" type="text"
								class="form-control picker-input pull-left"
								placeholder="${phFrom}" />
						...
						<div class="filter-submit pull-left">
							<input type="submit" class="btn-filter-submit"
						...
						<article class="events-list event-listing">
							...
							<c:if test="${not empty eventVO.images}">
								...
								<div class="events-content-wrapper">
									<p class="date-stamp">
										<c:out value="${eventVO.siteOpeningDate}" /> &nbsp; <spring:message code='label.to' /> &nbsp; <c:out value="${eventVO.siteEndingDate}" />
									</p>
									<p>
										<a
											href="/${param.languageCode}/${parent}/events/${eventVO.url}.html"><c:out
												value="${eventVO.teaserTitle}" /></a>
									</p>
								</div>
								<div class="events-link">
									<a
										href="/${param.languageCode}/${parent}/events/${eventVO.url}.html"
										class="events-location"><c:out
											value="${eventVO.addressLine1}" /></a>
								</div>
						</article>
				...
				<div class="listing-count">
			    	<c:if test="${not empty searchResponse.searchResults}">
				        ...
				        <span class="search-pagination-count">${fn:length(searchResponse.searchResults)}&nbsp;<spring:message code='label.of'/>&nbsp;${searchResponse.totalNumberOfrows}&nbsp;<spring:message code='search.results.label2'/></span>
			        </c:if>
							...
							<div>
								...
								<ul class="pagination">
									<c:if test="${searchResponse.currentPage > 1 }">
										<li><a
											href="/${param.languageCode}/${parent}/events.html?currentPage=${1}"
											class="first"><spring:message code="pagination.first" /></a></li>
									</c:if>
									<c:if test="${searchResponse.lowerLimitPagination > 1}">
										<li><a
											href="/${param.languageCode}/${parent}/events.html?currentPage=${searchResponse.lowerLimitPagination - 1}"
											...
											<c:if
												...
												<c:choose>
													<c:when
														...
														<li class="active"><a
															href="/${param.languageCode}/${parent}/events.html?currentPage=${pageCount}">${pageCount}</a></li>
													</c:when>
													<c:otherwise>
														<li><a
															href="/${param.languageCode}/${parent}/events.html?currentPage=${pageCount}">${pageCount}</a></li>
									...
									<c:if test="${searchResponse.totalNumberOfPages > count}">
										...
										<li><a href="/${param.languageCode}/${parent}/events.html?currentPage=${searchResponse.totalNumberOfPages}">${searchResponse.totalNumberOfPages}</a></li>

									</c:if>
									<c:if
										test="${searchResponse.upperLimitPagination < searchResponse.totalNumberOfPages}">
										<li><a
											href="/${param.languageCode}/${parent}/events.html?currentPage=${searchResponse.upperLimitPagination +1}"
											class="next">&raquo;</a></li>
									</c:if>
									<c:if
										test="${searchResponse.currentPage != searchResponse.totalNumberOfPages}">
										<li><a
											href="/${param.languageCode}/${parent}/events.html?currentPage=${searchResponse.totalNumberOfPages}"
