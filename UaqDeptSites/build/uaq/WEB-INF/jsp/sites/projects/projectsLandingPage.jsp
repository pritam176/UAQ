	...
	<div class="row">
		<div class="col-lg-9 col-sm-9 col-md-9 col-xs-12">
			...
			<div class="breadcrumbs hide-on-mobile">
				...
				<span><i class="glyphicon glyphicon-menu-right"></i></span>
				<spring:message code="label.department.projects" />
			</div>
			<!-- BreadCrumbs End -->
			...
			<div class="page-title-mob">
				...
				<c:forEach items="${searchResponse.searchProjectResults}"
					var="projectsVO">
					<article class="news-list project-list">
						<div class="row">
							<c:if test="${not empty projectsVO.image}">
								<div
									class="col-lg-3 col-sm-3 col-xs-12 events-img-wrapper">
									<img src="${projectsVO.image}" alt="Projects List">
								</div>
							</c:if>
							<div class="col-lg-9 col-sm-9 col-xs-12 events-content-wrapper">
								<h3 class="events-list-title">
									<a href="/${param.languageCode}${landing}/${projectsVO.url}">${projectsVO.teaserTitle}</a>
								</h3>
								<p class="date-stamp">${projectsVO.landingPageDate}</p>
								<p>
								${projectsVO.teaserText}
								<a href="/${param.languageCode}${landing}/${projectsVO.url}"><spring:message code="landing.readmore"/></a>
								</p>
							</div>
						</div>
					</article>
				...
				<div class="listing-count">
					<c:if test="${not empty searchResponse.searchProjectResults}">
						<span class="search-pagination-count-lbl"> <spring:message
								code='label.showing' />
						</span>
						<span class="search-pagination-count">${fn:length(searchResponse.searchProjectResults)}&nbsp;<spring:message
								code='label.of' />&nbsp;${searchResponse.totalNumberOfrows}&nbsp;<spring:message
								code='search.results.label2' /></span>
					</c:if>
					<c:if test="${empty searchResponse.searchProjectResults}">
						<span class="search-pagination-count-lbl"> <spring:message
								...
								<div>
									...
									<ul class="pagination">
										<c:if test="${searchResponse.currentPage > 1 }">
											<li><a
												href="/${param.languageCode}${landing}/projects.html?currentPage=${1}"
												class="first"><spring:message code="pagination.first" /></a></li>
										</c:if>
										<c:if test="${searchResponse.lowerLimitPagination > 1}">
											<li><a
												href="/${param.languageCode}${landing}/projects.html?currentPage=${searchResponse.lowerLimitPagination - 1}"
												...
												<c:if
													...
													<c:choose>
														<c:when
															...
															<li class="active"><a
																href="/${param.languageCode}${landing}/projects.html?currentPage=${pageCount}">${pageCount}</a></li>
														</c:when>
														<c:otherwise>
															<li><a
																href="/${param.languageCode}${landing}/projects.html?currentPage=${pageCount}">${pageCount}</a></li>
										...
										<c:if test="${searchResponse.totalNumberOfPages > count}">
											...
											<li><a
												href="/${param.languageCode}${landing}/projects.html?currentPage=${searchResponse.totalNumberOfPages}">${searchResponse.totalNumberOfPages}</a></li>

										</c:if>
										<c:if
											test="${searchResponse.upperLimitPagination < searchResponse.totalNumberOfPages}">
											<li><a
												href="/${param.languageCode}${landing}/projects.html?currentPage=${searchResponse.upperLimitPagination +1}"
												class="next">&raquo;</a></li>
										</c:if>
										<c:if
											test="${searchResponse.currentPage != searchResponse.totalNumberOfPages}">
											<li><a
												href="/${param.languageCode}${landing}/projects.html?currentPage=${searchResponse.totalNumberOfPages}"
