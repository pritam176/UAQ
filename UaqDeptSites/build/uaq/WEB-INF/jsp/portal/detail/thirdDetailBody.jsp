...
<c:if test="${not empty thirdLevelDetailPageVO.rhsIDViewObjectMap}">
	<c:forEach items="${thirdLevelDetailPageVO.rhsIDViewObjectMap}" var="rhsitmes">
...
<div class="col-md-12 mobile-landing-headding hidden-md">
	${parentLandingPage.title}
</div>
<div class="content">
	<div class="row">
		<!-- right col -->
		<div class="col-xs-12 col-sm-3 col-md-3 main-right-col">
					...
					<div class="social-share-wrap">
						<ul>
							...
							<li class="share-email">
								<c:set var="subjectValue" value="${thirdLevelDetailPageVO.title}"/>
			...
			<c:forEach items="${thirdLevelDetailPageVO.rhsIDViewObjectMap}" var="rhslist">
				<c:if test="${showImageGallery=='true' and not empty rhslist.value and rhslist.key=='ImageList'}">
								...
								<ul class="_gallery galleryUl col-md-12">
									<c:forEach items="${rhslist.value}" var="ImageVO">
										<c:if test="${not empty ImageVO.value}">
											...
											<li class="col-xs-4 col-sm-4 col-md-4 remove-pad"><a href="${imageAsset.galleryTeaserImage}"><img src="${imageAsset.originalImage}" alt=""></a></li>
										</c:if>
									</c:forEach>
								</ul>
								<a href="/${param.languageCode}/imageGallery.html" class="link-btn"><spring:message code="label.detail.view.photos" /></a>
							</div>
						</div>
					</div>
				</c:if>
				...
				<c:if test="${showRelatedAwards=='true' and not empty rhslist.value and rhslist.key=='AwardsList'}">
					<div class="callout-wrap">
						<div class="callout-head">
							<h5 class="right-nav-title">
			...
			<c:if test="${showArticleAsset=='true' and not empty thirdLevelDetailPageVO.rhsIDViewObjectMap}">
				<c:forEach items="${thirdLevelDetailPageVO.rhsIDViewObjectMap}" var="rhsList">
		...
		<div class="col-xs-12 col-sm-9 col-md-9 main-left-col">
			...
			<div class="page-title-wrap">
				<div class="row">
					<div class="col-xs-12 col-sm-9 col-md-10 title-txt">
						...
						<!-- /sub page title -->
						<!-- page title -->
						<h2 class="page-title">${thirdLevelDetailPageVO.title}</h2>
						<!-- /page title -->
					</div>
					<div class="hidden-xs show-sm col-md-2 title-icon">
						<img src="${thirdLevelDetailPageVO.titleIcon}" alt="uaq">
					</div>
				</div>
			</div>
			<!-- /page title -->
			<div class="main-content-wrap">
				<div class="sub-title">
					<h3>${thirdLevelDetailPageVO.heading}</h3>
				</div>
				<div class="page-content-wrap">
					<div class="content-wrap">
						<div class="common-content cf">
							<img src="${thirdLevelDetailPageVO.image}" class="img-left" />
							<p>${thirdLevelDetailPageVO.body}</p>
						</div>
							
							<c:if test="${not empty thirdLevelDetailPageVO.teaserImage}">
								<c:set var="teaserImage" value="${thirdLevelDetailPageVO.teaserImage}" />
							</c:if>
...
<!-- container fluid ends here -->
	<c:if test="${not empty thirdLevelDetailPageVO.title}">
		<c:set var="titleText" value="${thirdLevelDetailPageVO.title}"></c:set>
	</c:if>
	...
	<script type="text/javascript">
		...
		var sub_value = "${thirdLevelDetailPageVO.title}";
