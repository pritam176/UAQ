	...
	<div class="row">
		<div class="col-lg-9 col-sm-9 col-md-9 col-xs-12">
			...
			<div class="breadcrumbs hide-on-mobile">
				...
				<spring:message code="label.department.certificates"/>
			</div>
			<!-- BreadCrumbs End -->
			...
			<div class="page-title-mob">
				...
				<c:forEach items="${searchResponse.searchCertificateResults}"
					var="certificateVO">
					<article class="news-list award-listing">

						<div class="events-img-wrapper">
							<c:if test="${not empty certificateVO.images}">
								<c:forEach items="${certificateVO.images}" var="imageVO">
						...
						<div class="events-content-wrapper">
							<h3 class="events-list-title">
								<a href="/${param.languageCode}/${pageName}/certificate/${certificateVO.url}.html">${certificateVO.teaserTitle}</a>
							</h3>
							<p class="date-stamp">${certificateVO.landingPageDate}</p>
							<p>
							${certificateVO.teaserText}
							<a href="/${param.languageCode}/${pageName}/certificate/${certificateVO.url}.html"><spring:message code="landing.readmore"/></a>
							</p>
						</div>
					</article>
				</c:forEach>
				...
				<div class="listing-count">
					<c:if test="${not empty searchResponse.searchCertificateResults}">
						<span class="search-pagination-count-lbl"> <spring:message
								code='label.showing' />
						</span>
						<span class="search-pagination-count">
							${fn:length(searchResponse.searchCertificateResults)} &nbsp;<spring:message
								code='label.of' />&nbsp;${searchResponse.totalNumberOfrows}&nbsp;<spring:message
								code='search.results.label2' />
						</span>
					</c:if>
					<c:if test="${empty searchResponse.searchCertificateResults}">
						<span class="search-pagination-count-lbl"> <spring:message
								...
								<c:set var="count" value="1" />
									<ul class="pagination">
										<c:if test="${searchResponse.currentPage > 1 }">
											<li><a
												href="/${param.languageCode}/${pageName}/certificates.html?currentPage=${1}"
												class="first"><spring:message code="pagination.first" /></a></li>
										</c:if>
										<c:if test="${searchResponse.lowerLimitPagination > 1}">
											<li><a
												href="/${param.languageCode}/${pageName}/certificates.html?currentPage=${searchResponse.lowerLimitPagination - 1}"
												...
												<c:if
													...
													<c:choose>
														<c:when
															...
															<li class="active"><a
																href="/${param.languageCode}/${pageName}/certificates.html?currentPage=${pageCount}">${pageCount}</a></li>
														</c:when>
														<c:otherwise>
															<li><a
																href="/${param.languageCode}/${pageName}/certificates.html?currentPage=${pageCount}">${pageCount}</a></li>
										...
										<c:if test="${searchResponse.totalNumberOfPages > count}">
											...
											<li><a
												href="/${param.languageCode}/${pageName}/certificates.html?currentPage=${searchResponse.totalNumberOfPages}">${searchResponse.totalNumberOfPages}</a></li>

										</c:if>
										<c:if
											test="${searchResponse.upperLimitPagination < searchResponse.totalNumberOfPages}">
											<li><a
												href="/${param.languageCode}/${pageName}/certificates.html?currentPage=${searchResponse.upperLimitPagination +1}"
												class="next">&raquo;</a></li>
										</c:if>
										<c:if
											test="${searchResponse.currentPage != searchResponse.totalNumberOfPages}">
											<li><a
												href="/${param.languageCode}/${pageName}/certificates.html?currentPage=${searchResponse.totalNumberOfPages}"
