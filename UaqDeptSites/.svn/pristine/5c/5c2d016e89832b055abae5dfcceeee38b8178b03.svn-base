			...
			if (logininfo != null) {
				...
				if ((null != purchaseId) && (!purchaseId.isEmpty())) {
					try {
						...
						if(EXTENTION_OF_GRANT_LAND_REQUEST.equals(serviceId)){
							if(!StringUtil.isEmpty(calculateAmountFromService)){
								logger.debug("EXTENTION_OF_GRANT_LAND_REQUEST | added the Service return fee to Fee Detail class" + calculateAmountFromService);
								feeDetail.setServiceFee(Double.parseDouble(calculateAmountFromService));
							}
