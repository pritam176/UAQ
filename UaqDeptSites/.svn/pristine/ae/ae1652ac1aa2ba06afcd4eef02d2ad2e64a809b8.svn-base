...
public class EGDRequestService {
	...
	public LandOutputVO submitSupplierRequest(UserDeatilVO userDeatilVO, NewSupplierRegistrationVO supplierDetails) {
		createStub();

		LandOutputVO outputVO = new LandOutputVO();
		com.oracle.xmlns.UAQ_eGD_BusinessProcess.Supp_Registration_Renew.Supp_Registration_BPEL.ProcessType processType = new ProcessType();
		...
		commonRequestBPM.setStateId(1);
		commonRequestBPM.setRequestId(new Integer(0));
		commonRequestBPM.setRequestNo("");
		commonRequestBPM.setServiceName_Ar("");
		commonRequestBPM.setServiceName_En("");
		commonRequestBPM.setSupprReqId(new Integer(0));
		commonRequestBPM.setTemp_SupprReqId(new Integer(0));
		...
		com.oracle.xmlns.UAQ_eGD_BusinessProcess.Supp_Registration_Renew.Supp_Registration_BPEL.AttachmentRecPayload[] attachmentList = new AttachmentRecPayload[supplierDetails.getFileList().size()];
		...
		try {
			...
			if (output != null) {
				outputVO.setSatausId(output.getPaymentDetails().getIsPaymentRequired());
