...
@Component("awardsPageVOMapper")
public class AwardsPageVOMapper implements BaseVOMapper{

	public static transient UAQLogger logger = new UAQLogger(AwardsPageVOMapper.class);
	...
	public BaseVO mapAssetToVO(AssetBean assetBean) {
		...
		AwardsPageVO awardsPageVO = new AwardsPageVO();
		awardsPageVO.setAssetId(assetBean.getId().split(":")[1]);
		awardsPageVO.setName(assetBean.getName()); 
		...
		for (Attribute attribute : assetBean.getAttributes()) {
			  if (attribute.getName().equalsIgnoreCase("Title") && attribute.getData() != null) {
				  awardsPageVO.setTitle(attribute.getData().getStringValue());
			} else if (attribute.getName().equalsIgnoreCase("TitleIcon") && null!= attribute.getData()  && attribute.getData().getBlobValue() != null) {
				  awardsPageVO.setTitleIcon(AssetUtil.getBolbURL(attribute.getData().getBlobValue().getHref(), "image%2Fjpeg"));
		...
		awardsPageVO.setPageMetadataVO(pageMetadataVO);
		return awardsPageVO;
	}
