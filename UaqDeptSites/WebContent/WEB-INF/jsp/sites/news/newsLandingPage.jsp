						...
						<article class="news-list award-listing">
								...
								<div class="events-content-wrapper">
									<h3 class="events-list-title">
										<a href="/${param.languageCode}/${landing}/news/${newsList.url}.html">${newsList.teaserTitle}</a>
									</h3>
									<p class="date-stamp">${newsList.postedDate}</p>
							<p>
								${newsList.teaserText}
								<a href="/${param.languageCode}/${landing}/news/${newsList.url}.html"><spring:message code="landing.readmore"/></a>
							</p>
								</div>
						</article>
					</c:forEach>
							...
							<div>
								...
								<ul class="pagination">
									<c:if test="${searchResponse.currentPage > 1 }">
										<li><a
											href="/${param.languageCode}/${landing}/news-and-announcements.html?currentPage=${1}"
											class="first"><spring:message code="pagination.first" /></a></li>
									</c:if>
									<c:if test="${searchResponse.lowerLimitPagination > 1}">
										<li><a
											href="/${param.languageCode}/${landing}/news-and-announcements.html?currentPage=${searchResponse.lowerLimitPagination - 1}"
											...
											<c:if
												...
												<c:choose>
													<c:when
														...
														<li class="active"><a
															href="/${param.languageCode}/${landing}/news-and-announcements.html?currentPage=${pageCount}">${pageCount}</a></li>
													</c:when>
													<c:otherwise>
														<li><a
															href="/${param.languageCode}/${landing}/news-and-announcements.html?currentPage=${pageCount}">${pageCount}</a></li>
									...
									<c:if test="${searchResponse.totalNumberOfPages > count}">
										...
										<li><a href="/${param.languageCode}/${landing}/news-and-announcements.html?currentPage=${searchResponse.totalNumberOfPages}">${searchResponse.totalNumberOfPages}</a></li>

									</c:if>
									<c:if
										test="${searchResponse.upperLimitPagination < searchResponse.totalNumberOfPages}">
										<li><a
											href="/${param.languageCode}/${landing}/news-and-announcements.html?currentPage=${searchResponse.upperLimitPagination +1}"
											class="next">&raquo;</a></li>
									</c:if>
									<c:if
										test="${searchResponse.currentPage != searchResponse.totalNumberOfPages}">
										<li><a
											href="/${param.languageCode}/${landing}/news-and-announcements.html?currentPage=${searchResponse.totalNumberOfPages}"
