	...
	<div class="row">
		<div class="col-lg-9 col-sm-9 col-md-9 col-xs-12">
			...
			<div class="breadcrumbs hide-on-mobile">
				...
				<c:if test="${not empty parentLandingPage}">
					<span><i class="glyphicon glyphicon-menu-right"></i></span>
                        	<a href="/${param.languageCode}/aboutus/overview.html">${parentLandingPage.title}</a>
                        </c:if>
				<span><i class="glyphicon glyphicon-menu-right"></i></span>
			...
			<div class="page-title-mob">
				...
				<c:forEach items="${searchResponse.searchAwardResults}"
					var="awardVO">
					<article class="news-list award-listing">
							...
							<c:if test="${not empty awardVO.images}">
								<c:forEach items="${awardVO.images}" var="imageVO">
									<div class="listing-img">
										<img src="${imageVO.teaserImage}" alt="Award">
									</div>
								</c:forEach>
							</c:if>
						</div>
						...
						<div class="events-content-wrapper">
							<h3 class="events-list-title">
								<a href="/${param.languageCode}/${pageName}/award/${awardVO.url}.html">${awardVO.teaserTitle}</a>
							</h3>
							<p class="date-stamp"><fmt:formatDate  value="${awardVO.awardsDate}" pattern="dd MMMM yyyy"/></p>
							<p>
							${awardVO.teaserText}
							<a href="/${param.languageCode}/${pageName}/award/${awardVO.url}.html"><spring:message code="landing.readmore"/></a>
							</p>
						</div>
					</article>
				</c:forEach>
				...
				<div class="listing-count">
					<c:if test="${not empty searchResponse.searchAwardResults}">
						...
						<span class="search-pagination-count">
							${fn:length(searchResponse.searchAwardResults)} &nbsp;<spring:message
								code='label.of' />&nbsp;${searchResponse.totalNumberOfrows}&nbsp;<spring:message
								code='search.results.label2' />
						</span>
					</c:if>
					<c:if test="${empty searchResponse.searchAwardResults}">
						<span class="search-pagination-count-lbl"> <spring:message
								...
								<div>
									...
									<ul class="pagination">
										<c:if test="${searchResponse.currentPage > 1 }">
											<li><a
												href="/${param.languageCode}/${pageName}/awards-and-achievements.html?currentPage=${1}"
												class="first"><spring:message code="pagination.first" /></a></li>
										</c:if>
										<c:if test="${searchResponse.lowerLimitPagination > 1}">
											<li><a
												href="/${param.languageCode}/${pageName}/awards-and-achievements.html?currentPage=${searchResponse.lowerLimitPagination - 1}"
												...
												<c:if
													...
													<c:choose>
														<c:when
															...
															<li class="active"><a
																href="/${param.languageCode}/${pageName}/awards-and-achievements.html?currentPage=${pageCount}">${pageCount}</a></li>
														</c:when>
														<c:otherwise>
															<li><a
																href="/${param.languageCode}/${pageName}/awards-and-achievements.html?currentPage=${pageCount}">${pageCount}</a></li>
										...
										<c:if test="${searchResponse.totalNumberOfPages > count}">
											...
											<li><a
												href="/${param.languageCode}/${pageName}/awards-and-achievements.html?currentPage=${searchResponse.totalNumberOfPages}">${searchResponse.totalNumberOfPages}</a></li>

										</c:if>
										<c:if
											test="${searchResponse.upperLimitPagination < searchResponse.totalNumberOfPages}">
											<li><a
												href="/${param.languageCode}/${pageName}/awards-and-achievements.html?currentPage=${searchResponse.upperLimitPagination +1}"
												class="next">&raquo;</a></li>
										</c:if>
										<c:if
											test="${searchResponse.currentPage != searchResponse.totalNumberOfPages}">
											<li><a
												href="/${param.languageCode}/${pageName}/awards-and-achievements.html?currentPage=${searchResponse.totalNumberOfPages}"
