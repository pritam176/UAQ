	...
	<div class="row">
		<div class="col-lg-9 col-sm-9 col-md-9 col-xs-12">
			...
			<div class="breadcrumbs hide-on-mobile">
				...
				<span><i class="glyphicon glyphicon-menu-right"></i></span><spring:message code="label.our.people" />
			</div>
			<!-- BreadCrumbs End -->
			...
			<div class="page-title-mob">
				...
				<ul class="utility-wrap" style="float: right;">
					...
					<form:form method="POST" id="peopleForm" name="peopleForm"
						commandName="searchCommand" action="our-people.html">
				...
				<c:if test="${not empty searchResponse.searchPeopleResults}">

					<c:forEach items="${searchResponse.searchPeopleResults}" var="people">
						<article class="news-list award-listing">
								<div class="events-img-wrapper">
									<c:if test="${not empty people.image}">
								...
								<div class="events-content-wrapper">
									...
									<c:if test="${not empty people.position}">
										<p>
											${people.position}
										</p>
									</c:if>
				...
				<div class="listing-count">
			    	<c:if test="${not empty searchResponse.searchPeopleResults}">
				        ...
				        <span class="search-pagination-count">${fn:length(searchResponse.searchPeopleResults)}&nbsp;<spring:message code='label.of'/>&nbsp;${searchResponse.totalNumberOfrows}&nbsp;<spring:message code='search.results.label2'/></span>
			        </c:if>
			        <c:if test="${empty searchResponse.searchPeopleResults}">
			        	<span class="search-pagination-count-lbl">
							...
							<div>
								...
								<ul class="pagination">
									<c:if test="${searchResponse.currentPage > 1 }">
										<li><a
											href="/${param.languageCode}/${landing}/our-people.html?currentPage=${1}"
											class="first"><spring:message code="pagination.first" /></a></li>
									</c:if>
									<c:if test="${searchResponse.lowerLimitPagination > 1}">
										<li><a
											href="/${param.languageCode}/${landing}/our-people.html?currentPage=${searchResponse.lowerLimitPagination - 1}"
											...
											<c:if
												...
												<c:choose>
													<c:when
														...
														<li class="active"><a
															href="/${param.languageCode}/${landing}/our-people.html?currentPage=${pageCount}">${pageCount}</a></li>
													</c:when>
													<c:otherwise>
														<li><a
															href="/${param.languageCode}/${landing}/our-people.html?currentPage=${pageCount}">${pageCount}</a></li>
									...
									<c:if test="${searchResponse.totalNumberOfPages > count}">
										...
										<li><a href="/${param.languageCode}/${landing}/our-people.html?currentPage=${searchResponse.totalNumberOfPages}">${searchResponse.totalNumberOfPages}</a></li>

									</c:if>
									<c:if
										test="${searchResponse.upperLimitPagination < searchResponse.totalNumberOfPages}">
										<li><a
											href="/${param.languageCode}/${landing}/our-people.html?currentPage=${searchResponse.upperLimitPagination +1}"
											class="next">&raquo;</a></li>
									</c:if>
									<c:if
										test="${searchResponse.currentPage != searchResponse.totalNumberOfPages}">
										<li><a
											href="/${param.languageCode}/${landing}/our-people.html?currentPage=${searchResponse.totalNumberOfPages}"
